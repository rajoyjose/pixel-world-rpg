...
function updateQuests(type, data) {
  ...
  activeQuests.forEach(function (q) {
    if (q.done) return;
    if (type === "kill" && q.type === "kill" && q.monster === data.monster) {
      q.current = Math.min(q.current + 1, q.target);
      changed = true;
    }
    if (type === "collect" && q.type === "collect") {
      q.current = Math.min(q.current + 1, q.target);
      changed = true;
    }
    if (type === "level" && q.type === "level") {
      q.current = playerLevel;
      changed = true;
    }
    if (type === "boss" && q.type === "boss") {
      q.current = 1;
      changed = true;
    }
    if (type === "dimension" && q.type === "dimension") {
      q.current = 1;
      changed = true;
    }
    if (type === "survive" && q.type === "survive") {
      q.current = Math.floor(surviveTimer);
      changed = true;
    }
    if (type === "score" && q.type === "score") {
      q.current = score;
      changed = true;
    }
    if (!q.done && q.current >= q.target) {
      q.done = true;
      completeQuest(q);
    }
  });
...
function connectToServer() {
  try {
    ...
    playersRef.on("child_changed", function (snap) {
      ...
      if (otherPlayers[d.id] && otherPlayers[d.id].sprite) {
        ...
        op.tween = scene.tweens.add({
          ...
          onUpdate: function () {
            if (op.nameTag)
              op.nameTag.setPosition(op.sprite.x - 30, op.sprite.y - 52);
          },
        });
      }
    });
    ...
    chatRef.limitToLast(1).on("child_added", function (snap) {
      var d = snap.val();
      if (!d || d.senderId === myPlayerId) return;
...
function updateOnlineCount() {
  ...
  playersRef.once("value", function (snap) {
    updatePlayerCount(snap.numChildren());
  });
}
...
function startGame() {
  ...
  // Register touch functions
  window._doAttack = function () {
    doAttack();
  };
  window._doAbility = function () {
    doAbility();
  };
  window._doPotion = function () {
    doPotion();
  };
...
function startNextWave() {
  ...
  if (wi)
    wi.textContent =
      "🌊 Wave " + currentWave + " — " + waveEnemyCount + " enemies!";
  showFloatingText(
    "🌊 WAVE " + currentWave + "!",
    player.x - 60,
    player.y - 100,
  ...
  if (currentWave % 5 === 0)
    setTimeout(
      function () {
        spawnModeBoss("survival");
      },
      waveEnemyCount * 350 + 1000,
    );
}
  ...
  if (waveEnemiesKilled >= waveEnemyCount) {
    ...
    if (wi)
      wi.textContent =
        ...
        currentWave +
        " — " +
        (waveEnemyCount - waveEnemiesKilled) +
        " left!";
  }
}
...
function spawnBossRushBoss() {
  ...
  brg.fillStyle(cfg.color);
  brg.fillRoundedRect(5, 5, 90, 90, 12);
  brg.fillStyle(0xffffff, 0.3);
  brg.fillRoundedRect(15, 15, 70, 70, 8);
  brg.fillStyle(cfg.eyeColor);
  brg.fillCircle(30, 30, 14);
  brg.fillCircle(70, 30, 14);
  brg.fillStyle(0x000000);
  brg.fillCircle(30, 30, 8);
  brg.fillCircle(70, 30, 8);
  brg.fillStyle(cfg.eyeColor);
  brg.fillRect(25, 65, 50, 10);
  brg.generateTexture("rushBoss" + bossRushIndex, 100, 100);
  brg.destroy();
  ...
  bossHealthBar = scene.add
    .rectangle(640, 50, 500, 24, cfg.barColor)
    .setScrollFactor(0)
    .setDepth(15);
  bossNameText = scene.add
    .text(640, 50, cfg.name + " #" + bossRushIndex, {
      fontSize: "14px",
      fill: "#fff",
    })
    .setOrigin(0.5)
...
function spawnModeBoss(mode) {
  ...
  bg.fillStyle(cfg.color);
  bg.fillRoundedRect(5, 5, 90, 90, 10);
  bg.fillStyle(cfg.eyeColor);
  bg.fillCircle(30, 30, 15);
  bg.fillCircle(70, 30, 15);
  bg.fillStyle(0x000000);
  bg.fillCircle(30, 30, 9);
  bg.fillCircle(70, 30, 9);
  bg.fillStyle(cfg.mouthColor);
  bg.fillRect(25, 65, 50, 12);
  bg.generateTexture("modeBossTex", 100, 100);
  bg.destroy();
  ...
  bossHealthBarBg = scene.add
    ...
    .setDepth(15);
  bossHealthBar = scene.add
    .rectangle(640, 50, 500, 24, cfg.barColor || 0xff0000)
    .setScrollFactor(0)
    .setDepth(15);
  bossNameText = scene.add
    .text(640, 50, cfg.name + "  " + boss.health + "/" + boss.maxHealth, {
      fontSize: "14px",
      fill: "#fff",
    })
    .setOrigin(0.5)
...
function buildPvpDecorations(mapCfg) {
  ...
  // Center crystal
  var crystal = scene.add.triangle(
    (mapWidth / 2) * tileSize,
    (mapHeight / 2) * tileSize - 40,
    -20,
    30,
    20,
    30,
...
function createTextures() {
  ...
  opg.fillStyle(0x8844cc);
  opg.fillRoundedRect(4, 14, 32, 26, 5);
  opg.fillStyle(0xddbbff);
  opg.fillCircle(20, 10, 11);
  opg.fillStyle(0xaa66ff);
  opg.fillRect(1, 16, 8, 16);
  opg.generateTexture("otherPlayerTex", 42, 42);
  opg.destroy();
  ...
  Object.keys(SHADOW_MONSTERS).forEach(function (key) {
    ...
    smg.fillStyle(0xff0066, 0.5);
    smg.fillRect(Math.floor(m.size / 2) - 5, m.size - 14, 10, 4);
  ...
  potg.fillStyle(0x884400);
  potg.fillRect(10, 0, 10, 8);
...
function spawnEnemy(type) {
  ...
  var x = Phaser.Math.Between(4, mapWidth - 3) * tileSize,
    y = Phaser.Math.Between(4, mapHeight - 3) * tileSize;
...
function spawnShadowEnemy(type) {
  ...
  var x = Phaser.Math.Between(3, mapWidth - 2) * tileSize,
    y = Phaser.Math.Between(3, mapHeight - 2) * tileSize;
  var enemy = enemies.create(x, y, type + "Tex");
  enemy.setCollideWorldBounds(true);
  enemy.monsterType = type;
  enemy.isShadow = true;
...
function setupJoystick() {
  ...
  zone.addEventListener(
    ...
    function (e) {
      ...
      base.style.left = t.clientX - r.left - 55 + "px";
      base.style.bottom = "auto";
      base.style.top = t.clientY - r.top - 55 + "px";
    },
    { passive: false },
  );
  zone.addEventListener(
    ...
    function (e) {
      ...
      var t = e.changedTouches[0];
      var dx = t.clientX - joystickStartX,
        dy = t.clientY - joystickStartY,
...
function doSaveGame() {
  ...
  var popBorder = scene.add
    .rectangle(640, 360, 384, 204)
    .setStrokeStyle(3, 0x44ff44)
    .setFillStyle(0, 0);

  var title = scene.add
    .text(640, 300, "💾 GAME SAVED!", {
      fontSize: "28px",
      fill: "#44ff44",
      stroke: "#000",
      strokeThickness: 3,
  ...
  var info1 = scene.add
    .text(
      640,
      345,
      playerClass.icon + " " + playerName + "  —  Level " + playerLevel,
      {
        fontSize: "16px",
        fill: "#ffffff",
      },
  ...
  var info2 = scene.add
    .text(
      640,
      375,
      "Score: " + score + "   XP: " + playerXP + "/" + xpToNextLevel,
      {
        fontSize: "14px",
        fill: "#aaaaaa",
      },
  ...
  var info3 = scene.add
    .text(640, 400, "🎒 Inventory: " + inventory.length + " items", {
      fontSize: "13px",
      fill: "#ffaa44",
    })
    .setOrigin(0.5);
  ...
  var info4 = scene.add
    .text(640, 425, new Date().toLocaleString(), {
      fontSize: "11px",
      fill: "#666666",
    })
    .setOrigin(0.5);
...
function doAttack() {
  ...
  enemies.getChildren().forEach(function (e) {
    ...
    if (
      Phaser.Math.Distance.Between(player.x, player.y, e.x, e.y) <=
      playerAttackRange
    ) {
...
function spawnPotion(x, y, force) {
  ...
  var p = potions.create(x, y, "potionTex");
...
function abilityMage() {
  ...
  enemies.getChildren().forEach(function (e) {
    if (Phaser.Math.Distance.Between(player.x, player.y, e.x, e.y) < 270) {
      e.health -= playerDamage * 3.5;
      if (e.health <= 0) killEnemy(e);
    }
  });
  if (
...
function abilityPaladin() {
  ...
  enemies.getChildren().forEach(function (e) {
    if (
      ["skeleton", "wraith", "lich"].indexOf(e.monsterType) !== -1 &&
      Phaser.Math.Distance.Between(player.x, player.y, e.x, e.y) < 200
    ) {
      e.health -= playerDamage * 4;
      if (e.health <= 0) killEnemy(e);
    }
  });
  ...
  for (var i = 0; i < 3; i++) {
    (function (angle) {
      ...
      minion.label = scene.add
        .text(mx, my - 28, "☠️", { fontSize: "14px" })
        .setOrigin(0.5);
      scene.physics.add.overlap(minion, enemies, function (m, e) {
        if (!e || !e.active) return;
        e.health -= 15;
        if (e.health <= 0) killEnemy(e);
      });
...
function loadGame() {
  try {
    ...
    if (d.selectedClass) {
      window._selectedClass = d.selectedClass;
    }
  ...
  setTimeout(function () {
    scene.cameras.main.fade(2000, 0, 0, 0, false, function (cam, progress) {
      ...
      var modeLabel =
        {
          adventure: "🌍 Adventure",
          survival: "🌊 Survival — Wave " + currentWave,
          bossrush: "💀 Boss Rush — Boss #" + bossRushIndex,
          pvp: "⚔️ PvP Arena",
        }[selectedMode] || "";
      ...
      overlay.innerHTML =
        '<div style="text-align:center;max-width:560px;padding:40px;"><div style="font-size:90px;margin-bottom:16px;">💀</div><h1 style="font-size:60px;color:#ff2222;letter-spacing:6px;text-shadow:0 0 40px #ff0000;margin-bottom:8px;">GAME OVER</h1><p style="font-size:18px;color:#ffaa00;margin-bottom:4px;">' +
        modeLabel +
        '</p><p style="font-size:20px;color:#888;margin-bottom:6px;">' +
        (playerClass ? playerClass.icon : "") +
        " <b>" +
        playerName +
        "</b> — Level " +
        playerLevel +
        '</p><p style="font-size:28px;color:#ffdd44;margin-bottom:36px;">Score: ' +
        score +
...
function update() {
  ...
  potions.getChildren().forEach(function (p) {
    if (p.label) p.label.setPosition(p.x - 8, p.y - 28);
  });
  ...
  minions.forEach(function (m) {
    ...
    var dx = player.x - m.x,
      ...
      dist = Math.sqrt(dx * dx + dy * dy);
    if (dist > 80 && dist > 0) {
      m.setVelocityX((dx / dist) * m.speed);
      m.setVelocityY((dy / dist) * m.speed);
  ...
  wolfPets.forEach(function (wolf) {
    ...
    if (closest) {
      var wdx = closest.x - wolf.x,
        ...
        wd = Math.sqrt(wdx * wdx + wdy * wdy);
      if (wd > 0) {
        wolf.setVelocityX((wdx / wd) * wolf.speed);
        wolf.setVelocityY((wdy / wd) * wolf.speed);
      }
    } else {
      ...
      if (pd > 100 && pd > 0) {
        wolf.setVelocityX((pdx / pd) * 150);
        wolf.setVelocityY((pdy / pd) * 150);
  ...
  enemies.getChildren().forEach(function (e) {
    ...
    var dx = player.x - e.x,
      ...
      dist = Math.sqrt(dx * dx + dy * dy);
    if (dist < 280 && dist > 0) {
      e.setVelocityX((dx / dist) * e.speed);
      e.setVelocityY((dy / dist) * e.speed);
    } else {
      var pdx = (e.patrolX || e.x) - e.x,
        pdy = (e.patrolY || e.y) - e.y,
        pdist = Math.sqrt(pdx * pdx + pdy * pdy);
      if (pdist > 100 && pdist > 0) {
        e.setVelocityX((pdx / pdist) * e.speed * 0.4);
        e.setVelocityY((pdy / pdist) * e.speed * 0.4);
      } else if (Math.random() < 0.008) {
        e.setVelocityX(Phaser.Math.Between(-40, 40));
        e.setVelocityY(Phaser.Math.Between(-40, 40));
      }
    }
    var mSize = e.isShadow
      ? (SHADOW_MONSTERS[e.monsterType] || SHADOW_MONSTERS.wraith).size || 40
      : (MONSTERS[e.monsterType] || MONSTERS.goblin).size || 40;
    if (e.healthBar) {
      e.healthBar.setPosition(e.x, e.y - 35);
      e.healthBar.width = mSize * (e.health / e.maxHealth);
    }
