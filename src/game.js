...
window.touchAttack = function () {
  if (gameStarted && player) playerAttack();
};
window.touchAbility = function () {
  if (gameStarted && player) useAbility();
};
window.touchPotion = function () {
...
function initQuests() {
  activeQuests = QUEST_TEMPLATES.slice(0, 5).map(function (q) {
    return Object.assign({}, q, { current: 0, done: false });
  });
  updateQuestPanel();
}
...
function completeQuest(quest) {
  ...
  setTimeout(function () {
    ...
    var next = QUEST_TEMPLATES.find(function (q) {
      return !doneIds.includes(q.id);
    });
    if (next) {
      ...
      if (idx !== -1) {
        activeQuests[idx] = Object.assign({}, next, {
          current: 0,
          done: false,
        });
      }
...
function buildWorld() {
  ...
  treePts.forEach(function (pos) {
    ...
    var ty = pos.y * tileSize + tileSize / 2;
    var trunk = scene.add.rectangle(tx, ty, 16, 28, 0x6b3a2a);
    var top = scene.add.circle(tx, ty - 14, 26, 0x1a6e1a);
    var top2 = scene.add.circle(tx - 8, ty - 20, 18, 0x228822);
    normalTiles.push(trunk, top, top2);
  });
  ...
  [
    ...
    var fy = pos.y * tileSize + tileSize / 2;
  ...
  [
    ...
    var cx = pos.x * tileSize + tileSize / 2;
    var cy = pos.y * tileSize + tileSize / 2;
    var c1 = scene.add
      .triangle(cx, cy - 10, -8, 15, 8, 15, 0, -20, 0x8800ff)
      .setVisible(false);
    scene.tweens.add({
      targets: c1,
      alpha: 0.4,
...
function createTextures() {
  ...
  // Shadow monsters
  Object.entries(SHADOW_MONSTERS).forEach(function (entry) {
    var key = entry[0];
    var m = entry[1];
...
function setupJoystick() {
  ...
  zone.addEventListener(
    ...
    function (e) {
      ...
      var dy = touch.clientY - joystickStartY;
...
function playerAttack() {
  ...
  var startAngle = playerFacing === 1 ? -0.5 : Math.PI + 0.5;
  var endAngle = playerFacing === 1 ? 0.8 : Math.PI - 0.8;
  swing.arc(player.x, player.y, playerAttackRange * 0.7, startAngle, endAngle);
  swing.strokePath();
  scene.tweens.add({
    targets: swing,
    alpha: 0,
  ...
  enemies.getChildren().forEach(function (enemy) {
    ...
    if (dist <= playerAttackRange) {
      ...
      if (selectedClass === "berserker") {
        var hpRatio = playerHealth / playerMaxHealth;
        if (hpRatio < 0.5) dmg = Math.floor(dmg * 1.8);
        if (hpRatio < 0.25) dmg = Math.floor(dmg * 2.5);
      }
      var crit = false;
      if (selectedClass === "rogue" && Math.random() < 0.3) {
  ...
  if (boss && boss.active) {
    var bDist = Phaser.Math.Distance.Between(
      player.x,
      player.y,
      boss.x,
      boss.y,
    );
    if (bDist <= playerAttackRange + 50) {
  ...
  for (var i = 0; i < 16; i++) {
    ...
    (function (a) {
      enemies.getChildren().forEach(function (enemy) {
        ...
        var ey = enemy.y - player.y;
        var dist = Math.sqrt(ex * ex + ey * ey);
        if (dist < 280 && Math.abs(Math.atan2(ey, ex) - a) < 0.25) {
...
function abilityPaladin() {
  ...
  enemies.getChildren().forEach(function (enemy) {
    if (["skeleton", "wraith", "lich"].includes(enemy.monsterType)) {
      if (
...
function abilityNecromancer() {
  ...
  for (var i = 0; i < 3; i++) {
    ...
    var my = player.y + Math.sin(angle) * 60;
    var minion = scene.physics.add.image(mx, my, "minionTex");
    minion.health = 40 + playerLevel * 10;
    minion.speed = 130;
    minions.push(minion);
    var mLabel = scene.add
    ...
    (function (mn) {
      scene.time.delayedCall(15000, function () {
        if (mn && mn.active) {
          spawnParticles(mn.x, mn.y, 0x44ff44, 8);
          if (mn.label) mn.label.destroy();
          mn.destroy();
...
function abilityRanger() {
  ...
  for (var i = 0; i < 2; i++) {
    ...
    var wy = player.y + Phaser.Math.Between(-60, 60);
...
function updateMinimap() {
  ...
  var playerColor = playerClass
    ? "#" + playerClass.color.toString(16).padStart(6, "0")
    : "#4488ff";
...
function showGameOverScreen() {
  ...
  overlay.style.cssText =
    ...
    "z-index:9999;opacity:0;transition:opacity 1.5s ease;" +
    "font-family:Arial,sans-serif;color:white;";

  overlay.innerHTML =
    '<div style="text-align:center;max-width:560px;padding:40px;">' +
    '<div style="font-size:90px;margin-bottom:16px;">💀</div>' +
    '<h1 style="font-size:60px;color:#ff2222;letter-spacing:6px;' +
    'text-shadow:0 0 40px #ff0000;margin-bottom:8px;">GAME OVER</h1>' +
    ...
    score +
    "</p>" +
    '<p style="font-size:14px;color:#444;font-style:italic;margin-bottom:36px;">' +
    "Your tombstone remains where you fell..." +
    "</p>" +
    '<div style="display:flex;gap:16px;justify-content:center;">' +
    '<button id="restartBtn" style="padding:14px 36px;font-size:18px;' +
    "background:linear-gradient(135deg,#aa0000,#dd2222);color:white;" +
    'border:2px solid #ff4444;border-radius:10px;cursor:pointer;font-family:Arial;">⚔️ Play Again</button>' +
    '<button id="menuBtn" style="padding:14px 36px;font-size:18px;' +
    "background:#1a1a1a;color:white;border:2px solid #444;" +
    'border-radius:10px;cursor:pointer;font-family:Arial;">🏠 Main Menu</button>' +
    "</div>" +
    '<p style="margin-top:18px;font-size:12px;color:#333;">Press R to play again</p>' +
    "</div>";
...
function update() {
  ...
  // Sprint
  if (((shiftKey && shiftKey.isDown) || touchSprinting) && stamina > 0) {
    speed *= 1.55;
    isSprintActive = true;
  ...
  // Walking bob
  if (moving) {
    player.y += Math.sin(Date.now() / 120) * 0.5;
  }
  ...
  if (playerNameText) {
    playerNameText.setPosition(player.x - 40, player.y - 58);
  }

  // Update potion labels
  ...
  enemies.getChildren().forEach(function (enemy) {
    ...
    if (dist < 280 && dist > 0) {
      ...
      var pdy2 = (enemy.patrolY || enemy.y) - enemy.y;
    ...
    if (enemy.nameTag) {
      enemy.nameTag.setPosition(enemy.x, enemy.y - 50);
    }
  });
}
