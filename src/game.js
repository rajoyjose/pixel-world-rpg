...
var isMobile = /Android|iPhone|iPad|iPod|Opera Mini|IEMobile/i
    .test(navigator.userAgent) || navigator.maxTouchPoints > 1;
...
var CLASSES = {
    warrior: {
        name: 'Warrior', icon: '⚔️', color: 0xff6666,
        hp: 220, speed: 155, damage: 35, attackRange: 80,
        ability: 'Shield Bash', abilityIcon: '🛡️',
        abilityCooldown: 5000, attackCooldown: 700
    },
    archer: {
        name: 'Archer', icon: '🏹', color: 0x66ff66,
        hp: 110, speed: 285, damage: 28, attackRange: 200,
        ability: 'Arrow Rain', abilityIcon: '🏹',
        abilityCooldown: 4000, attackCooldown: 500
    },
    mage: {
        name: 'Mage', icon: '🧙', color: 0x6688ff,
        hp: 80, speed: 200, damage: 70, attackRange: 150,
        ability: 'Fireball', abilityIcon: '🔥',
        abilityCooldown: 6000, attackCooldown: 900
    },
    rogue: {
        name: 'Rogue', icon: '🗡️', color: 0xffaa44,
        hp: 110, speed: 265, damage: 50, attackRange: 70,
        ability: 'Vanish', abilityIcon: '👁️',
        abilityCooldown: 8000, attackCooldown: 400
    },
    paladin: {
        name: 'Paladin', icon: '⚜️', color: 0xffddaa,
        hp: 180, speed: 155, damage: 28, attackRange: 90,
        ability: 'Holy Aura', abilityIcon: '✨',
        abilityCooldown: 7000, attackCooldown: 800
    },
    necromancer: {
        name: 'Necromancer', icon: '💀', color: 0x44ff88,
        hp: 90, speed: 195, damage: 40, attackRange: 130,
        ability: 'Raise Dead', abilityIcon: '☠️',
        abilityCooldown: 9000, attackCooldown: 800
    },
    berserker: {
        name: 'Berserker', icon: '🪓', color: 0xff2222,
        hp: 180, speed: 195, damage: 55, attackRange: 85,
        ability: 'Berserk Rage', abilityIcon: '😤',
        abilityCooldown: 10000, attackCooldown: 500
    },
    ranger: {
        name: 'Ranger', icon: '🌿', color: 0x44ffaa,
        hp: 130, speed: 245, damage: 32, attackRange: 160,
        ability: 'Wolf Pack', abilityIcon: '🐺',
        abilityCooldown: 12000, attackCooldown: 600
    }
};
...
var QUEST_TEMPLATES = [
    ...
    { id:'q5',  desc:'Kill 3 Orcs',            type:'kill',     monster:'orc',      target:3,   reward:75,  xpReward:65  },
    { id:'q6',  desc:'Collect 3 Potions',      type:'collect',                      target:3,   reward:50,  xpReward:40  },
    { id:'q7',  desc:'Reach Level 3',          type:'level',                        target:3,   reward:150, xpReward:100 },
    { id:'q8',  desc:'Reach Level 5',          type:'level',                        target:5,   reward:300, xpReward:200 },
    { id:'q9',  desc:'Defeat the Dragon King', type:'boss',                         target:1,   reward:500, xpReward:400 },
    { id:'q10', desc:'Enter Shadow Realm',     type:'dimension',                    target:1,   reward:100, xpReward:80  },
    { id:'q11', desc:'Kill 3 Wraiths',         type:'kill',     monster:'wraith',   target:3,   reward:120, xpReward:100 },
    { id:'q12', desc:'Survive 2 minutes',      type:'survive',                      target:120, reward:200, xpReward:150 },
    { id:'q13', desc:'Score 500 points',       type:'score',                        target:500, reward:180, xpReward:120 }
];
...
var activeQuests = [];
var questStats = {
    kills: {}, potionsCollected: 0,
    bossKills: 0, dimensionVisited: 0,
    surviveTime: 0
};
...
window.playerAttack = function() { if (gameStarted && player) playerAttack(); };
window.useAbility   = function() { if (gameStarted && player) useAbility(); };
window.usePotion    = function() { if (gameStarted && player) usePotion(); };
...
var sounds = {
    ...
    save:      function() { playTone(440, 0.1, 'sine'); },
    ability:   function() { playTone(300, 0.3, 'square'); },
    portal:    function() {
        playTone(200, 0.2, 'sine');
        setTimeout(function() { playTone(400, 0.3, 'sine'); }, 200);
    },
    questDone: function() {
        playTone(660, 0.1, 'sine');
        setTimeout(function() { playTone(880, 0.2, 'sine'); }, 150);
    },
    bossSpawn: function() {
        playTone(100, 0.5, 'sawtooth');
        setTimeout(function() { playTone(80, 0.5, 'sawtooth'); }, 300);
    },
    gameOver: function() {
        playTone(400, 0.4, 'sine');
        setTimeout(function() { playTone(350, 0.4, 'sine'); }, 400);
        setTimeout(function() { playTone(300, 0.4, 'sine'); }, 800);
        setTimeout(function() { playTone(250, 0.4, 'sine'); }, 1200);
        setTimeout(function() { playTone(200, 0.8, 'sine'); }, 1600);
    }
};
...
function initQuests() {
    activeQuests = QUEST_TEMPLATES.slice(0, 5).map(function(q) {
        return { id:q.id, desc:q.desc, type:q.type, monster:q.monster,
                 target:q.target, reward:q.reward, xpReward:q.xpReward,
...
function updateQuests(type, data) {
    ...
    activeQuests.forEach(function(quest) {
        ...
        if (type === 'kill' && quest.type === 'kill' && quest.monster === data.monster) { quest.current = Math.min(quest.current + 1, quest.target); changed = true; }
        if (type === 'collect' && quest.type === 'collect') { quest.current = Math.min(quest.current + 1, quest.target); changed = true; }
        if (type === 'level' && quest.type === 'level') { quest.current = playerLevel; changed = true; }
        if (type === 'boss' && quest.type === 'boss') { quest.current = 1; changed = true; }
        if (type === 'dimension' && quest.type === 'dimension') { quest.current = 1; changed = true; }
        if (type === 'survive' && quest.type === 'survive') { quest.current = Math.floor(surviveTimer); changed = true; }
        if (type === 'score' && quest.type === 'score') { quest.current = score; changed = true; }
        if (!quest.done && quest.current >= quest.target) { quest.done = true; completeQuest(quest); }
    });
...
function completeQuest(quest) {
    ...
    if (player) showFloatingText('📜 Quest Done! +' + quest.reward, player.x - 100, player.y - 100, '#ffaa00');
    addChatMessage('Quest', '✅ ' + quest.desc, '#ffaa00');
    setTimeout(function() {
        var doneIds = activeQuests.map(function(q) { return q.id; });
        for (var i = 0; i < QUEST_TEMPLATES.length; i++) {
            if (doneIds.indexOf(QUEST_TEMPLATES[i].id) === -1) {
                var next = QUEST_TEMPLATES[i];
                var idx = activeQuests.findIndex(function(q) { return q.id === quest.id; });
...
function updateQuestPanel() {
    ...
    activeQuests.forEach(function(quest) {
        ...
        div.innerHTML = (quest.done ? '✅ ' : '🔲 ') + quest.desc +
            '<div class="questProgress">' + quest.current + '/' + quest.target + '</div>' +
            '<div class="questReward">+' + quest.reward + ' pts  +' + quest.xpReward + ' XP</div>';
...
function connectToServer() {
    try {
        myPlayerId = 'player_' + Date.now() + '_' + Math.floor(Math.random() * 9999);
...
function updateOnlineCount() {
    ...
    playersRef.once('value', function(snapshot) { updatePlayerCount(snapshot.numChildren()); });
}
...
function addTombstoneVisual(x, y, name, level, color) {
    ...
    var label = scene.add.text(x, y - 34, '💀 ' + name + '\nLv.' + level,
        { fontSize: '10px', fill: color || '#cccccc', stroke: '#000', strokeThickness: 2, align: 'center' }
...
function buildWorld() {
    for (var x = 0; x < mapWidth; x++) {
        for (var y = 0; y < mapHeight; y++) {
            var color;
            if (Math.abs(x - y) < 2 || Math.abs(x + y - mapWidth) < 2) { color = 0x8B6914; }
            else if ((x + y) % 2 === 0) { color = 0x2d8a2d; }
            else { color = 0x267326; }
    ...
    for (var i = 5; i < 15; i++) {
        var water = scene.add.rectangle(22*tileSize+tileSize/2, i*tileSize+tileSize/2, tileSize-1, tileSize-1, 0x1155aa);
        normalTiles.push(water);
    }

    var treePts = [{x:2,y:2},{x:3,y:4},{x:5,y:3},{x:8,y:1},{x:9,y:3},{x:15,y:4},{x:16,y:6},{x:18,y:7},{x:3,y:12},{x:4,y:14},{x:20,y:15},{x:21,y:17},{x:10,y:20},{x:11,y:22},{x:7,y:8},{x:12,y:5},{x:1,y:18},{x:25,y:5},{x:26,y:7},{x:28,y:12}];
    ...
    [{x:10,y:10},{x:20,y:5},{x:5,y:22}].forEach(function(pos) {
        ...
        var flame1 = scene.add.triangle(fx, fy-20, -10, 10, 10, 10, 0, -15, 0xff4400);
        var flame2 = scene.add.triangle(fx-6, fy-12, -6, 8, 6, 8, 0, -10, 0xff8800);
        scene.tweens.add({ targets: [flame1, flame2], scaleX: 0.85, scaleY: 1.15, duration: 150+Math.random()*100, yoyo: true, repeat: -1 });
    ...
    for (var sx = 0; sx < mapWidth; sx++) {
        for (var sy = 0; sy < mapHeight; sy++) {
            ...
            shadowTiles.push(scene.add.rectangle(sx*tileSize+tileSize/2, sy*tileSize+tileSize/2, tileSize-1, tileSize-1, sc).setVisible(false));
        }
    }

    [{x:3,y:3},{x:8,y:6},{x:14,y:2},{x:20,y:8}].forEach(function(pos) {
        var c1 = scene.add.triangle(pos.x*tileSize+tileSize/2, pos.y*tileSize+tileSize/4, -8, 15, 8, 15, 0, -20, 0x8800ff).setVisible(false);
        scene.tweens.add({ targets: c1, alpha: 0.4, duration: 800, yoyo: true, repeat: -1 });
        shadowTiles.push(c1);
    });
...
function createTextures() {
    ...
    pg.fillStyle(playerClass.color); pg.fillRoundedRect(4,12,32,28,5);
    pg.fillStyle(0xffddaa); pg.fillCircle(20,10,12);
    pg.generateTexture('playerTex',40,42); pg.destroy();

    var opg = scene.make.graphics({x:0,y:0,add:false});
    opg.fillStyle(0xaa44ff); opg.fillRoundedRect(4,12,32,28,5);
    opg.fillStyle(0xffddaa); opg.fillCircle(20,10,12);
...
function createPortals() {
    ...
    var pDefs = [
        { x:24*tileSize, y:24*tileSize, color:0xaa00ff, label:'🌀 Shadow Portal' },
        { x:4*tileSize,  y:4*tileSize,  color:0xffaa00, label:'🌀 Return Portal' }
    ];
...
function spawnEnemy(type) {
    ...
    enemy.monsterType = type; enemy.isShadow = false;
...
function spawnShadowEnemy(type) {
    ...
    var enemy = enemies.create(x, y, type+'Tex');
    enemy.setCollideWorldBounds(true);
    enemy.monsterType = type; enemy.isShadow = true;
    enemy.health = m.hp*(1+playerLevel*0.2); enemy.maxHealth = enemy.health;
    enemy.speed = m.speed+playerLevel*4; enemy.damage = m.damage;
    enemy.xpReward = m.xp+playerLevel*5; enemy.scoreValue = m.score;
    enemy.patrolX = x; enemy.patrolY = y;
...
function setupKeyboard() {
    ...
    scene.input.keyboard.on('keydown-SPACE', function() { if (!chatOpen) playerAttack(); });
    scene.input.keyboard.on('keydown-I',     function() { if (!chatOpen) toggleInventory(); });
    scene.input.keyboard.on('keydown-F',     function() { if (!chatOpen) usePotion(); });
    scene.input.keyboard.on('keydown-E',     function() { if (!chatOpen) useAbility(); });
    scene.input.keyboard.on('keydown-T',     function() { openChat(); });
    scene.input.keyboard.on('keydown-ENTER', function() { if (!chatOpen) { saveGame(); sounds.save(); showFloatingText('💾 Saved!', player.x, player.y-60, '#00ffff'); } });
}
...
function setupJoystick() {
    ...
    zone.addEventListener('touchstart', function(e) {
        ...
        joystickActive = true; joystickStartX = touch.clientX; joystickStartY = touch.clientY;
        var zoneRect = zone.getBoundingClientRect();
        base.style.left = (touch.clientX-zoneRect.left-55)+'px'; base.style.bottom = 'auto'; base.style.top = (touch.clientY-zoneRect.top-55)+'px';
    }, { passive: false });
    zone.addEventListener('touchmove', function(e) {
        e.preventDefault(); if (!joystickActive) return;
        var touch = e.changedTouches[0];
        var dx = touch.clientX-joystickStartX, dy = touch.clientY-joystickStartY;
...
function playerAttack() {
    ...
    enemies.getChildren().forEach(function(enemy) {
        ...
        if (Phaser.Math.Distance.Between(player.x, player.y, enemy.x, enemy.y) <= playerAttackRange) {
            ...
            scene.time.delayedCall(100, function() { if(enemy&&enemy.active) enemy.clearTint(); });
            showFloatingText((crit?'⚡ CRIT! ':'')+dmg, enemy.x, enemy.y-40, crit?'#ffff00':'#ffddaa');
            hitAny = true;
...
function spawnPotion(x, y, force) {
    ...
    scene.tweens.add({ targets: potion, y: potion.y-12, duration: 600, yoyo: true, repeat: -1 });
    potion.label = scene.add.text(x-18, y-28, '💊 Potion', { fontSize:'10px', fill:'#ffaacc' });
    return potion;
}
...
function abilityWarrior() {
    ...
    enemies.getChildren().forEach(function(enemy) {
        if (Phaser.Math.Distance.Between(player.x,player.y,enemy.x,enemy.y)<160) {
            var prev=enemy.speed; enemy.speed=0; enemy.setTint(0xffff00); enemy.health-=playerDamage*2.5;
            scene.time.delayedCall(2500, function() { if(enemy&&enemy.active){enemy.speed=prev;enemy.clearTint();} });
            if (enemy.health<=0) killEnemy(enemy);
        }
    });
}
...
function abilityArcher() {
    ...
    for (var i=0; i<16; i++) {
        (function(angle) {
            ...
            enemies.getChildren().forEach(function(enemy) {
                var ex=enemy.x-player.x, ey=enemy.y-player.y, dist=Math.sqrt(ex*ex+ey*ey);
                if (dist<280 && Math.abs(Math.atan2(ey,ex)-angle)<0.25) { enemy.health-=playerDamage; if(enemy.health<=0)killEnemy(enemy); }
            });
        })((i/16)*Math.PI*2);
    }
}
...
function abilityMage() {
    ...
    enemies.getChildren().forEach(function(enemy) {
        if (Phaser.Math.Distance.Between(player.x,player.y,enemy.x,enemy.y)<270) { enemy.health-=playerDamage*3.5; if(enemy.health<=0)killEnemy(enemy); }
    });
...
function abilityPaladin() {
    ...
    enemies.getChildren().forEach(function(enemy) {
        if (['skeleton','wraith','lich'].indexOf(enemy.monsterType)!==-1) {
            if (Phaser.Math.Distance.Between(player.x,player.y,enemy.x,enemy.y)<200) { enemy.health-=playerDamage*4; if(enemy.health<=0)killEnemy(enemy); }
        }
    });
...
function abilityNecromancer() {
    ...
    for (var i=0; i<3; i++) {
        (function(angle) {
            ...
            minion.label = scene.add.text(mx, my-28, '☠️ Minion', { fontSize:'10px', fill:'#88ff88', stroke:'#000', strokeThickness:2 }).setOrigin(0.5);
            scene.physics.add.overlap(minion, enemies, function(m,enemy) { if(!enemy||!enemy.active)return; enemy.health-=15; if(enemy.health<=0)killEnemy(enemy); });
...
function abilityRanger() {
    ...
    for (var i=0; i<2; i++) {
        (function() {
            ...
            wolf.label = scene.add.text(wx, wy-28, '🐺 Wolf', { fontSize:'10px', fill:'#44ffaa', stroke:'#000', strokeThickness:2 }).setOrigin(0.5);
            scene.physics.add.overlap(wolf, enemies, function(w,enemy) {
                if (!enemy||!enemy.active)return;
                var now=Date.now(); if(!w.lastBite||now-w.lastBite>800){w.lastBite=now;enemy.health-=20+playerLevel*3;if(enemy.health<=0)killEnemy(enemy);}
            });
...
function spawnBoss() {
    ...
    boss.setCollideWorldBounds(true); boss.health=600+playerLevel*120; boss.maxHealth=boss.health; boss.speed=55+playerLevel*5; boss.xpReward=400+playerLevel*60;
    ...
    bossHealthBar   = scene.add.rectangle(640,50,500,24,0xff0000).setScrollFactor(0).setDepth(15);
    bossNameText    = scene.add.text(640,50,'💀 DRAGON KING  '+boss.health+'/'+boss.maxHealth,{fontSize:'14px',fill:'#fff'}).setOrigin(0.5).setScrollFactor(0).setDepth(15);
    showFloatingText('⚠️ THE DRAGON KING APPEARS!', player.x-160, player.y-100, '#ff0000');
...
function killBoss() {
    ...
    scene.time.addEvent({ delay:180000, callback:function(){spawnBoss();} });
}

function updateBossHealthBar() {
    if(!boss||!bossHealthBar)return;
    bossHealthBar.width=500*(boss.health/boss.maxHealth);
    if(bossNameText) bossNameText.setText('💀 DRAGON KING  '+Math.max(0,Math.floor(boss.health))+'/'+boss.maxHealth);
}
...
function startDayNightCycle() {
    ...
    scene.time.addEvent({ delay:35000, loop:true, callback:function() {
        ...
        if (dayTime===1) {
            ...
            enemies.getChildren().forEach(function(e){e.speed*=1.25;});
...
function buildUI() {
    ...
    var staminaBg = scene.add.rectangle(16,62,206,10,0x111111).setOrigin(0,0).setScrollFactor(0).setDepth(10);
    ...
    if (!isMobile) {
        scene.add.text(16,696,'WASD:Move  SHIFT:Sprint  SPACE:Attack  E:'+playerClass.ability+'  F:Potion  I:Bag  T:Chat  Enter:Save',{fontSize:'10px',fill:'#88aa88'}).setScrollFactor(0).setDepth(10);
    }
...
function updateInventoryPanel() {
    ...
    if (inventory.length===0) {
        var e=scene.add.text(640,360,'No items yet...',{fontSize:'16px',fill:'#555'}).setOrigin(0.5).setScrollFactor(0).setDepth(31);
        inventoryTexts.push(e); return;
    }
    var counts={};
    inventory.forEach(function(i){counts[i]=(counts[i]||0)+1;});
    var y=275;
    Object.keys(counts).forEach(function(item) {
        var t=scene.add.text(640,y,'💊 '+item+'  ×'+counts[item],{fontSize:'18px',fill:'#ffffff'}).setOrigin(0.5).setScrollFactor(0).setDepth(31);
        inventoryTexts.push(t); y+=44;
    });
}
    ...
    input.onkeydown=function(e) {
        if (e.key==='Enter') {
            ...
            if (msg) {
                ...
                if (chatRef&&myPlayerId) chatRef.push({ name:playerClass.icon+' '+playerName, message:msg, senderId:myPlayerId, time:Date.now() });
            }
            input.value=''; input.disabled=true; chatOpen=false; input.blur();
        }
...
function addChatMessage(name, msg, color) {
    ...
    var div=document.createElement('div');
    div.style.color=color||'#fff'; div.style.marginBottom='3px';
...
function loadGame() {
    try {
        ...
        if(!saved)return;
        var data=JSON.parse(saved); if(!data)return;
        playerLevel=data.playerLevel||1; playerXP=data.playerXP||0;
        playerHealth=data.playerHealth||100; playerMaxHealth=data.playerMaxHealth||100;
        playerSpeed=data.playerSpeed||200; playerDamage=data.playerDamage||20;
        xpToNextLevel=data.xpToNextLevel||100; score=data.score||0;
        inventory=data.inventory||[]; playerName=data.playerName||'Hero';
...
function showGameOver() {
    player.setVelocity(0); player.setActive(false); gameStarted=false;
    ...
    setTimeout(function() {
        scene.cameras.main.fade(2000,0,0,0,false,function(cam,progress) {
            ...
            overlay.innerHTML=
                ...
                '<p style="font-size:20px;color:#888;margin-bottom:6px;">'+(playerClass?playerClass.icon:'')+' <b>'+playerName+'</b> — Level '+playerLevel+'</p>'+
                '<p style="font-size:28px;color:#ffdd44;margin-bottom:36px;">Score: '+score+'</p>'+
            ...
            function restartGame() {
                ...
                minions=[];wolfPets=[];isBerserking=false;playerInvisible=false;gameStarted=false;
                localStorage.removeItem('pixelRPGSave');
                scene.cameras.main.fadeIn(100);
                scene.scene.restart();
                setTimeout(function(){var menu=document.getElementById('startMenu');if(menu)menu.style.display='flex';},500);
            }
...
function update() {
    ...
    if ((shiftKey&&shiftKey.isDown||window.touchSprinting)&&stamina>0) {
        speed*=1.55; stamina=Math.max(0,stamina-0.8);
    } else {
    ...
    potions.getChildren().forEach(function(p){if(p.label)p.label.setPosition(p.x-18,p.y-28);});
    ...
    minions.forEach(function(minion) {
        ...
        var mdx=player.x-minion.x,mdy=player.y-minion.y,mdist=Math.sqrt(mdx*mdx+mdy*mdy);
        if(mdist>80&&mdist>0){minion.setVelocityX((mdx/mdist)*minion.speed);minion.setVelocityY((mdy/mdist)*minion.speed);}
    ...
    enemies.getChildren().forEach(function(enemy) {
        ...
        var dx=player.x-enemy.x,dy=player.y-enemy.y,dist=Math.sqrt(dx*dx+dy*dy);
        if(dist<280&&dist>0){enemy.setVelocityX((dx/dist)*enemy.speed);enemy.setVelocityY((dy/dist)*enemy.speed);}
        else{
            var pdx2=(enemy.patrolX||enemy.x)-enemy.x,pdy2=(enemy.patrolY||enemy.y)-enemy.y,pdist2=Math.sqrt(pdx2*pdx2+pdy2*pdy2);
            if(pdist2>100&&pdist2>0){enemy.setVelocityX((pdx2/pdist2)*enemy.speed*0.4);enemy.setVelocityY((pdy2/pdist2)*enemy.speed*0.4);}
            else if(Math.random()<0.008){enemy.setVelocityX(Phaser.Math.Between(-40,40));enemy.setVelocityY(Phaser.Math.Between(-40,40));}
        }
        var mSize=enemy.isShadow?((SHADOW_MONSTERS[enemy.monsterType]||SHADOW_MONSTERS.wraith).size||40):((MONSTERS[enemy.monsterType]||MONSTERS.goblin).size||40);
        if(enemy.healthBar){enemy.healthBar.setPosition(enemy.x,enemy.y-35);enemy.healthBar.width=mSize*(enemy.health/enemy.maxHealth);}
