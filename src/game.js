...
// ================================
var CLASSES = {
    warrior:     { name:'Warrior',     icon:'⚔️',  color:0xff6666, hp:220, speed:155, damage:35, attackRange:80,  ability:'Shield Bash', abilityIcon:'🛡️', abilityCooldown:5000,  attackCooldown:700 },
    archer:      { name:'Archer',      icon:'🏹',  color:0x66ff66, hp:110, speed:285, damage:28, attackRange:200, ability:'Arrow Rain',  abilityIcon:'🏹', abilityCooldown:4000,  attackCooldown:500 },
    mage:        { name:'Mage',        icon:'🧙',  color:0x6688ff, hp:80,  speed:200, damage:70, attackRange:150, ability:'Fireball',    abilityIcon:'🔥', abilityCooldown:6000,  attackCooldown:900 },
    rogue:       { name:'Rogue',       icon:'🗡️', color:0xffaa44, hp:110, speed:265, damage:50, attackRange:70,  ability:'Vanish',      abilityIcon:'👁️',abilityCooldown:8000,  attackCooldown:400 },
    paladin:     { name:'Paladin',     icon:'⚜️', color:0xffddaa, hp:180, speed:155, damage:28, attackRange:90,  ability:'Holy Aura',   abilityIcon:'✨', abilityCooldown:7000,  attackCooldown:800 },
    necromancer: { name:'Necromancer', icon:'💀',  color:0x44ff88, hp:90,  speed:195, damage:40, attackRange:130, ability:'Raise Dead',  abilityIcon:'☠️', abilityCooldown:9000,  attackCooldown:800 },
...
var QUEST_TEMPLATES = [
    ...
    { id:'q8',  desc:'Defeat Dragon King',     type:'boss',                         target:1,   reward:500, xpReward:400 },
    { id:'q9',  desc:'Enter Shadow Realm',     type:'dimension',                    target:1,   reward:100, xpReward:80  },
    { id:'q10', desc:'Survive 2 minutes',      type:'survive',                      target:120, reward:200, xpReward:150 },
    { id:'q11', desc:'Score 500 points',       type:'score',                        target:500, reward:180, xpReward:120 },
    { id:'q12', desc:'Kill 3 Wraiths',         type:'kill',     monster:'wraith',   target:3,   reward:120, xpReward:100 }
];
...
var pvpMode = false;
var bossRushIndex = 0;
var bossRushBosses = ['fire','ice','thunder','dark','dragon'];

// ================================
// REGISTER FUNCTIONS FOR TOUCH/HTML
// This is the KEY fix — no more loops!
// ================================
...
document.getElementById('startBtn').onclick = function() {
    ...
    gameStarted = true;
    console.log('Starting', selectedMode, 'as', selectedClass);
};
...
var sounds = {
    ...
    pvpKill:  function(){ playTone(700,0.2,'sine'); setTimeout(function(){playTone(900,0.3,'sine');},150); },
...
function connectToServer() {
    try {
        ...
        myRef.set({ id:myPlayerId, name:playerClass.icon+' '+playerName, x:400, y:300, level:playerLevel, dimension:0, targetX:400, targetY:300 });
...
function addOtherPlayer(data) {
    ...
    // targetX/Y used for smooth interpolation
...
function sendPosition() {
    ...
    var now = Date.now();
    if(now - lastPosSent < 50) return; // Send max 20 times/second
    lastPosSent = now;
...
function startGame() {
    ...
    // Register touch functions AFTER playerClass is set
...
// ================================
// BOSS RUSH MODE — bosses only!
// ================================
function startBossRushMode() {
    bossRushIndex = 0;
    showFloatingText('💀 BOSS RUSH! No breaks!', player.x-120, player.y-80, '#ff4444');
    setTimeout(function(){ spawnBossRush(); }, 2000);
}
...
function spawnBossRush() {
    ...
    var bossColors = [0xff4400, 0x0044ff, 0xffff00, 0x440088, 0x660000];
    var bossNames  = ['🔥 Fire Titan', '❄️ Ice Giant', '⚡ Storm Lord', '🌑 Shadow King', '💀 Dragon King'];
    var bossColor  = bossColors[(bossRushIndex-1) % bossColors.length];
    var bossName   = bossNames[(bossRushIndex-1)  % bossNames.length];

    // Make rush boss texture
    var brg = scene.make.graphics({x:0,y:0,add:false});
    brg.fillStyle(bossColor); brg.fillRoundedRect(5,5,90,90,10);
    brg.fillStyle(0xffffff); brg.fillCircle(28,28,14); brg.fillCircle(72,28,14);
    brg.fillStyle(0x000000); brg.fillCircle(28,28,8);  brg.fillCircle(72,28,8);
    brg.generateTexture('rushBossTex'+bossRushIndex, 100, 100); brg.destroy();

    boss = scene.physics.add.image(x, y, 'rushBossTex'+bossRushIndex);
    boss.setCollideWorldBounds(true);
    boss.health = 300 + (bossRushIndex*200) + playerLevel*50;
    boss.maxHealth = boss.health;
    boss.speed = 70 + bossRushIndex*10 + playerLevel*5;
    boss.xpReward = 200 + bossRushIndex*100;
    bossSpawned = true;
    ...
    bossHealthBar   = scene.add.rectangle(640,50,500,24,bossColor).setScrollFactor(0).setDepth(15);
    bossNameText    = scene.add.text(640,50,bossName+' #'+bossRushIndex, {fontSize:'14px',fill:'#fff'}).setOrigin(0.5).setScrollFactor(0).setDepth(15);

    showFloatingText('⚠️ '+bossName, player.x-100, player.y-100, '#ff4444');
...
function startPvpMode() {
    // Arena style — no potions spawning from enemies
    // Players can damage each other
    addChatMessage('System','⚔️ PvP Mode! Fight other players!','#ff88ff');
    showFloatingText('⚔️ PvP Arena!', player.x-60, player.y-80, '#ff88ff');
    // Spawn some enemies too for fun
    spawnInitialEnemies();
}
...
function buildWorld() {
    for(var x=0;x<mapWidth;x++){
        for(var y=0;y<mapHeight;y++){
            ...
            normalTiles.push(scene.add.rectangle(x*tileSize+tileSize/2,y*tileSize+tileSize/2,tileSize-1,tileSize-1,color));
        }
    }

    for(var i=5;i<15;i++) normalTiles.push(scene.add.rectangle(22*tileSize+tileSize/2,i*tileSize+tileSize/2,tileSize-1,tileSize-1,0x1155aa));

    [{x:2,y:2},{x:3,y:4},{x:5,y:3},{x:8,y:1},{x:15,y:4},{x:16,y:6},{x:18,y:7},{x:3,y:12},{x:10,y:20},{x:7,y:8},{x:12,y:5},{x:1,y:18},{x:25,y:5},{x:28,y:12}].forEach(function(pos){
...
// ================================
// ATTACK — doAttack() is the main function
// ================================
function doAttack() {
    var now=Date.now();
    ...
    enemies.getChildren().forEach(function(enemy){
        ...
        if(Phaser.Math.Distance.Between(player.x,player.y,enemy.x,enemy.y) <= playerAttackRange){
            ...
            showFloatingText((crit?'⚡ ':'')+dmg, enemy.x, enemy.y-40, crit?'#ffff00':'#ffddaa');
            hitAny=true;
    ...
    if(pvpMode) {
        Object.values(otherPlayers).forEach(function(op){
            ...
            if(Phaser.Math.Distance.Between(player.x,player.y,op.sprite.x,op.sprite.y)<=playerAttackRange){
                sounds.pvpKill();
                showFloatingText('⚔️ HIT!', op.sprite.x, op.sprite.y-40, '#ff88ff');
                // Send PvP hit via Firebase
                if(chatRef&&myPlayerId){
                    chatRef.push({ name:'System', message:'⚔️ '+playerClass.icon+' '+playerName+' hit '+op.name+'!', senderId:myPlayerId, time:Date.now() });
                }
            }
        });
    }

    if(!hitAny&&!boss) showFloatingText('miss',player.x+playerFacing*30,player.y-20,'#666666');
}

// ================================
// PLAYER TOUCHED ENEMY (collision)
// ================================
...
function spawnPotion(x,y,force) {
    ...
    p.label=scene.add.text(x-18,y-28,'💊 Potion',{fontSize:'10px',fill:'#ffaacc'});
    return p;
}
...
// RENAMED to doPotion — no more self-calling loop!
function doPotion() {
...
// ================================
// ABILITIES — doAbility() is the main function
// ================================
function doAbility() {
    var now=Date.now();
...
function abilityMage() {
    ...
    enemies.getChildren().forEach(function(e){
        if(Phaser.Math.Distance.Between(player.x,player.y,e.x,e.y)<270){ e.health-=playerDamage*3.5; if(e.health<=0)killEnemy(e); }
    });
...
function abilityPaladin() {
    ...
    enemies.getChildren().forEach(function(e){
        if(['skeleton','wraith','lich'].indexOf(e.monsterType)!==-1){
            if(Phaser.Math.Distance.Between(player.x,player.y,e.x,e.y)<200){ e.health-=playerDamage*4; if(e.health<=0)killEnemy(e); }
        }
    });
...
function abilityRanger() {
    ...
    for(var i=0;i<2;i++){
        (function(){
            ...
            scene.physics.add.overlap(wolf,enemies,function(w,e){
                if(!e||!e.active) return;
                var now=Date.now(); if(!w.lastBite||now-w.lastBite>800){w.lastBite=now;e.health-=20+playerLevel*3;if(e.health<=0)killEnemy(e);}
            });
...
function spawnBoss() {
    ...
    showFloatingText('⚠️ DRAGON KING!',player.x-100,player.y-100,'#ff0000');
...
function killBoss() {
    ...
    if(selectedMode==='bossrush'){
        // Spawn next boss rush boss!
        setTimeout(function(){spawnBossRush();},3000);
    } else {
...
function buildInventoryPanel() {
    ...
    inventoryPanel.setScrollFactor(0).setDepth(30); inventoryPanel.setVisible(false);
}
...
function toggleInventory() {
    inventoryOpen=!inventoryOpen; inventoryPanel.setVisible(inventoryOpen);
...
function showGameOver() {
    ...
    setTimeout(function(){
        scene.cameras.main.fade(2000,0,0,0,false,function(cam,progress){
            ...
            var modeLabel={adventure:'🌍 Adventure',survival:'🌊 Survival - Wave '+currentWave,bossrush:'💀 Boss Rush - Boss #'+bossRushIndex,pvp:'⚔️ PvP Arena'};
            overlay.innerHTML=
                ...
                '<p style="font-size:18px;color:#ffaa00;margin-bottom:4px;">'+modeLabel[selectedMode]+'</p>'+
...
function update() {
    ...
    Object.values(otherPlayers).forEach(function(op){
        ...
        var lerpSpeed = 0.15; // 0 = no movement, 1 = instant
        var dx = op.targetX - op.sprite.x;
        var dy = op.targetY - op.sprite.y;
        op.sprite.x += dx * lerpSpeed;
        op.sprite.y += dy * lerpSpeed;
    ...
    minions.forEach(function(m){
        ...
        if(dist>80&&dist>0){m.setVelocityX((dx/dist)*m.speed);m.setVelocityY((dy/dist)*m.speed);}
